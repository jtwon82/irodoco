           ...
           <div id="admin-index" class='bodycenter' dojotype="dijit.layout.ContentPane" region="center" style='z-index:0;'>
				<div style=''>
					<div class="page-header">
					  <h1>Facility Maintenance Modify & Add<small> </small></h1>
					</div>
				</div>
				<br>
<form name="form1" action="/admin-facility-history-proc" method="post" enctype="multipart/form-data">
	...
	<input type="hidden" name="CCTV_ID" value="${ map.CCTV_ID }">
				...
				<div class="input-group">
				  ...
				  <input type="text" name="HIST_ID" data-subname="ID" class="form-control" aria-describedby="basic-addon2" value='${ item.HIST_ID }' readOnly>
				</div>
				<br>
				<div class="input-group">
				  <span class="input-group-addon" id="basic-addon2">Maintenance Type</span>
				  <select name="HIST_TYPE" data-subname="Maintenance Type" class="form-control" aria-describedby="basic-addon2">
				  <option value="">-- Select --</option>
				  <option value="1">전원상태</option>
				  <option value="2">통신상태</option>
				  <option value="3">모듈상태</option>
				  </select>
				  <script type="text/javascript">
				  document.form1.HIST_TYPE.value='${item.HIST_TYPE}';
				  </script>
				</div>
				<br>
				<div class="input-group">
				  <span class="input-group-addon" id="basic-addon2">Check Type</span>
				  <input type="text" name="HIST_STATUS" data-subname="Check Type" class="form-control" aria-describedby="basic-addon2" value='${ item.HIST_STATUS }'>
				</div>
				<br>
				<div class="input-group">
				  <span class="input-group-addon" id="basic-addon2">Comment</span>
				  <textarea name="HIST_COMMENT" data-subname="Comment" class="form-control" aria-describedby="basic-addon2">${ item.HIST_COMMENT }</textarea>
				</div>
				<br>
				<br>
				<br>
	...
	<script type="text/javascript">
		function submitChk(){
			...
			if ( f.HIST_TYPE.value=='' ){
				alert( 'Please insert to '+ $(f.HIST_TYPE).data('subname') +'');
				return false;
			}
			if ( f.HIST_STATUS.value=='' ){
				alert( 'Please insert to '+ $(f.HIST_STATUS).data('subname') +'');
				return false;
			}
			if ( f.HIST_COMMENT.value=='' ){
				alert( 'Please insert to '+ $(f.HIST_COMMENT).data('subname') +'');
				return false;
			}
			return true;
		}
