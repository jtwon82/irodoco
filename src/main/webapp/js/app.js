require(
		[ 'dojo/query', 'dojo/on', 'dojo/dom', 'dojo/dom-construct',
				'dojo/dom-geometry', 'dojo/_base/connect', 'dojo/has',
				'dojo/_base/event', 'dojo/dom-class', 'dojo/dom-style',
				'esri/domUtils', 'esri/map', 'dojo/NodeList-dom',
				'dojo/domReady!' ],
		function($, on, dom, domConstruct, domGeom, connect, has, event,
				domClass, domStyle, domUtils) {
			...
			esri.config.defaults.io.corsEnabledServers
					.push('https://developers.arcgis.com/javascript');
			...
			function bindEventHandlers() {
				...
				$update.attr('disabled', 'disabled').on('click', updateSandbox);
			...
			function getSampleCode() {
				...
				esri.request({
					// 'url':
					// 'https://developers.arcgis.com/javascript/3/samples/' +
					// sampleFolder + '/index.html',
			...
			function updateSandbox() {
				...
				domUtils.show(dom.byId('loader')); // Show a loading/updating
													// icon
				...
				idoc = (iframe.contentWindow) ? iframe.contentWindow
				...
				if (relLinks) {
					...
					relLinks = relLinks.concat(content.match(/src=".+?"/g)
							.filter(function(url) {
								return url.indexOf("//") === -1
							}));
					relLinks.forEach(function(href) {
						var path = href.split('"')[1];
						content = content.replace(path, "../samples/"
								+ sampleFolder + "/" + path);
					});
				}
				...
				if (relLinks) {
					relLinks.forEach(function(href) {
						content = content.replace(href, "../samples/"
								+ sampleFolder + "/" + href);
					});
				}
				;

				// fix for syntax used in data_gas_prices and
				...
				if (relLinks) {
					...
					relLinks.forEach(function(href) {
						...
						content = content.replace(file, "../samples/"
								+ sampleFolder + "/" + file);
					});
				}
				;
				...
				if (relLinks) {
					...
					relLinks.forEach(function(href) {
						var file = href.split('"')[3]; // more "s than in fix
														// above
						content = content.replace(file, "../samples/"
								+ sampleFolder + "/" + file);
					});
				...
				// fix for syntax used in ed_undoRedo, graphics_undoRedo and
				// gp_clipasync
				...
				if (jsLinks) {
					jsLinks.forEach(function(href) {
						content = content.replace(/""\)/, "\"/../samples/"
								+ sampleFolder + "/\")");
						content = content.replace(/''\)/, "\'/../samples/"
								+ sampleFolder + "/\')");
					});
				...
				if (has('ie')) {
					...
					// this workaround was causing issues in Firefox - may be
					// able to get it to
					// work cross-browser need to test more. But for now this
					// 'split' approach works.
					...
					// otherwise, no referrer is sent when making requests from
					// the iframe
					// which breaks samples which use the /sproxy on
					// developers.arcgis.com
					// TODO temporary fix (Chris), need to go back and check
					// what this is doing
					// iframe.src = '../samples/' + sampleFolder +
					// '/index.html';
				}
			}
			...
			function saveFile() {
				...
				var blob = builder.getBlob('text/plain;charset='
			...
			function log(str) {
				if (DEBUG && console && console.log) { // check for console
														// first
